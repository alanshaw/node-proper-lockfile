...
function updateLock(file, options) {
    ...
    lock.updateTimeout = setTimeout(() => {
        ...
        options.fs.stat(getLockFile(file, options), (err, stat) => {
            ...
            if (!isMtimeOurs) {
              console.error({ isMtimeOurs }, lock.mtime.getTime(), stat.mtime.getTime())
              console.error({mtimeMs: stat.mtimeMs})
            ...
            options.fs.utimes(getLockFile(file, options), mtime, mtime, (err) => {
                ...
                if (lock.released) {
                  console.error('updated utimes but lock released')
                    return;
                }
                ...
                if (err) {
                    ...
                    console.error('error updating utimes to', mtime.getTime())
                    return updateLock(file, options);
                }
